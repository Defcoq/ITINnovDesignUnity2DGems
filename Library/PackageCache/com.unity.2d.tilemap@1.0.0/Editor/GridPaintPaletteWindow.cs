...
namespace UnityEditor.Tilemaps
{
    internal class GridPaintPaletteWindow : EditorWindow
    {
        ...
        class SceneTilemapOverlay : TransientSceneViewOverlay
        {
            const string k_OverlayId = "Scene View/Tilemap";
            const string k_DisplayName = "Tilemap";
            GridPaintPaletteWindow m_Window;
            public override bool visible
            {
                get { return GridPaintingState.defaultBrush != null && GridPaintingState.scenePaintTarget != null; }
            }
            ...
            public override void OnGUI()
            {
                if (m_Window == null)
                {
                    var wins = Resources.FindObjectsOfTypeAll(typeof(GridPaintPaletteWindow)) as GridPaintPaletteWindow[];
                    m_Window = wins.Length > 0 ? wins[0] : null;
                }

                if (m_Window != null)
                {
                    m_Window.DisplayFocusMode();
                }
            }
        }
    }
